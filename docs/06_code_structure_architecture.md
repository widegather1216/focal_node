...
```text
focal_node/
├── src-tauri/               # Tauri(Rust) 백엔드 및 앱 패키징 로직 (구현됨)
│   ├── tauri.conf.json      # Tauri 앱 설정 및 Sidecar 바이너리 매핑 설정
│   └── src/main.rs          # Python 사이드카 프로세스 기동 및 포트 파싱 로직
├── src/                     # React/Vite 프론트엔드 코드 (구현됨)
├── backend/                 # Python FastAPI 백엔드 (AI 추론 / DB 관리)
│   ├── app/
│   │   ├── api/             # 분리된 FastAPI 라우터 모듈 모음
│   │   │   ├── chat.py      # 포트폴리오 비평 및 AI 챗봇 API
│   │   │   ├── folders.py   # 폴더 관리(조회, 삭제) API
│   │   │   ├── indexing.py  # 백그라운드 인덱싱 작업 큐 관리 API
│   │   │   ├── photos.py    # 갤러리 렌더링 및 메타데이터 단건 조회 API
...
│   │   ├── utils/
│   │   │   └── image.py            # RAW 파일 디코딩 및 exifread를 통한 EXIF 데이터 추출 병합 모듈
│   │   ├── config.py               # 개발/운영 환경 판별 및 데이터 영구 저장 경로(~/.config/focal_node) 동적 설정
│   │   ├── database.py             # SQLAlchemy 접속(WAL 모드, FK 강제) 엔진
│   │   ├── models.py               # SQLite ORM 스키마 (images, image_metadata, ai_analysis, indexed_folders 등)
│   │   ├── chroma.py               # ChromaDB 로컬 Persistent 설정
│   │   └── main.py                 # FastAPI 진입점 (포트 0번 동적 할당 및 라우터 등록)
...
└── ~/.config/focal_node/    # (config.py에 의해 생성되는 운영 데이터 폴더)
...
### 3.1. `main.py` 및 `config.py` (진입점 및 환경 설정)
*   **`config.py`:** `sys.frozen` 값을 확인해 PyInstaller로 패키징된 배포 환경인지 판단하고, DB와 캐시가 저장될 데이터 폴더를 운영체제의 설정 디렉토리(`~/.config/focal_node`)로 안전하게 격리(Data Isolation)합니다.
*   **`main.py`:** `uvicorn.run(port=0)`으로 실행되어 OS 유휴 포트를 부여받고, 해당 포트를 `[Sidecar] PORT: {port}` 규격으로 stdout에 출력합니다. 내부적으로 `api/` 디렉토리에 있는 모든 세부 모듈 라우터(Router)들을 수합하여 등록합니다.
