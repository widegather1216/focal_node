...
# 1. PyInstaller로 백엔드 빌드 (가상환경 활성화 상태, --onedir 디렉터리 모드)
python3 backend/build_backend.py

# 2. 빌드된 사이드카 바이너리 및 _internal 디렉토리를 Tauri 사이드카 디렉토리로 복사 (Apple Silicon 기준)
cp dist/focal_node_backend/focal_node_backend src-tauri/binaries/focal_node_backend-aarch64-apple-darwin
rm -rf src-tauri/binaries/_internal
cp -r dist/focal_node_backend/_internal src-tauri/binaries/_internal
